<?php
$l_subTit = "Jouw plek om Spaans te leren";

$l_botInicio = "Home";
$l_botProfe = "De lerares";
$l_botCursos = "Lessen";
$l_botContacto = "Contact";

$l_inicio = <<<inicio
<p>Welkom!</p>
            <p>Als je geinteresseerd bent in de Spaanse taal dan ben je hier aan het juiste adres.</p>
            <p>Bij Spanish Online leer je niet alleen de taal maar ook de cultuur, de historie, het toerisme, de kunst en de muziek kennen!</p>
            <p>We bieden originele en interactieve <a #cursos>lessen</a>, afgestemd op de specifieke behoeftes en interesses van de student.</p>
            <p>Spaans is in alle gevallen de moedertaal van de <a #profe>lerares</a>. Bovendien heeft zij jarenlange ervaring in het begeleiden van studenten met het leren van de Spaanse taal.</p>
            <p>Meer informatie? Neem gerust <a #contacto>contact</a> met me op.</p>
inicio;

$l_profe = <<<profe
            <h4>De lerares<img id="im_vero" src="img/vero/vero.jpg" alt="mi_pic" /></h4>
            <p>Hallo! Mijn naam is Verónica, ik kom uit Buenos Aires, Argentinië, en ik geef al sinds meer dan 6 jaar met veel plezier en passie les in Spaans als vreemde taal.</p>
            <br /><br />
            <p>Op de universteit van Buenos Aires begon ik met literatuur, waar ik met lof afstudeerde. Vervolgens ontving ik een studie beurs om een master te doen genaamd “Learning and teaching Spanish in multilingual and international contexts”. Hierdoor kreeg ik de mogelijkheid om in Spanje, de Verenigde Staten en Nederland te wonen. Ik heb ook de cursus  “Basisvorming voor Spaans leraren” van het Instituut Cervantes, dat wereldwijd de Spaanse taal promoot.</p>
            <br />
            <p>In het jaar 2007 begon ik als lerares te werken. Sindsdien heb ik ontmoet studenten van verschillende oorsprong, leeftijden, niveaus en interesses. Naast het hebben van een diepe kennis over de gepubliceerde boeken om Spaans te leren, heb ik ook bereid educatieve materialen die speciaal zijn bestemd om de behoeften van mijn studenten.</p>
            <br />
            <p>Mijn studenten noemen me een vrolijke geduldige lerares met ontzettend veel drive voor lesgeven.</p>
            <br />
            <p><a #contacto>Hoop je binnenkort te leren kennen!</a></p>
profe;

$l_cursos = <<<cursos
            <h2>Jouw Spaanse les</h2>
            <h4>Over focus en methodologie</h4>
            <br />
            <p>Alle lessen hebben een focus op communicatie. Door de globalisatie is communicatie de sleutel tot de toekomst geworden. De lessen vereisen dan ook een actieve houding van de student die vanaf de eerste dag Spaans zal spreken.</p>
            <br />
            <p>Bovendien is het belangrijk om de cultuur te leren kennen achter een taal. Het zijn juist die details van het dagelijkse leven, de mzuiek, film, de manier van uitdrukken die een talencursus een echte uitwisseling van cultuur maken. In de les gebruiken we authentieke en actuele onderwerpen van verschillende landen waar Spaans wordt gesproken, en in verschillende formats: liedjes, videos, kranten, blogs, foto’s, etc.</p>
            <br />
            <p>De filosofie is dat elke student uniek is: heeft zijn eigen interesses en leerstijl. Het is daarom essentieel om de student te leren kennen en de cursus aan te passen naar gelang de persoonlijke doelen van de student.</p>
            <br />
            <h4>Wij bieden:</h4>
            <ul>
                <li>priveles, les in koppels of in groepen</li>
                <li>generieke en specifieke lessen (spreekvaardigheid, gramatica, voorbereiding op examens, literatuur, kunst, etc.)</li>
                <li>lessen in persoon of online (skype).</li>
            </ul>
            <p> Voor vragen over prijzen en speciale pakketten kunt u <a #contacto>contact</a> opnemen</p>
cursos;

$l_contacto = <<<contacto
            <h4>Meer informatie?</h4>
            <p>Neem gerust contact met me op.</p>
            <br /><br /><br />
contacto;

$l_tel = "Telefoon: ";

$l_lenguaje = "Taal:";

